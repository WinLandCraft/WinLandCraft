...
Other Fabric mods can register native, Chromium, and hybrid apps through the versioned public API. See [PLUGINS.md](PLUGINS.md) for the complete development guide, input contract, file associations, examples, and compatibility policy. The installable example is maintained in the separate `WLCP-Example-Plugin` project; this build produces only the host mod, sources, and compile-only API JAR.
