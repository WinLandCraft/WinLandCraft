...
Settings apply live and persist in `config/winlandcraft.json`. Remote input is relayed only while the checkbox is enabled, is bound to the active stream and dimension, and cannot reposition or close the replica. Controller identity, packet bounds, and rate are validated by the server; held keys/buttons are released on timeout, disconnect, or permission changes. System-modifier shortcuts are not forwarded, so a remote player cannot invoke the creator's clipboard shortcuts. Video is H.264 (Annex B) with one-second keyframes for late joins and recovery; the sender probes GPU encoders (NVIDIA NVENC, AMD AMF, Intel Quick Sync, Apple VideoToolbox) before falling back to software x264, and the receiver decodes in software. Audio is stereo 48 kHz Opus. The selected tab of the currently shared browser supplies audio through CEF's browser-scoped audio handler. Desktop audio, microphone input, private browsers, and inactive tabs are not broadcast. Because CEF capture replaces native audio output, each browser tab has a bounded Java Sound playback queue for local playback, including private and background tabs. Only the active tab of the published app feeds the encoder; native apps have no audio source.

Encoding and playback use a bundled FFmpeg (JavaCPP presets, no system install or new port needed) through pure-Java `StreamEncoder`/`StreamDecoder` workers. The sender captures the composed panel to RGBA, converts to YUV 4:2:0, and encodes H.264 plus Opus into the existing bounded `StreamMedia` envelopes; receivers decode to an RGBA texture upload and Java Sound playback. Queues stay bounded: stale video is discarded until a replacement keyframe while audio is preserved. Encoder failures appear in the app/log with the probed candidates instead of falling back silently.

No Chromium GPU flags are involved anymore: FFmpeg talks to the vendor driver directly (NVENC/AMF/QSV/VideoToolbox when present, x264 otherwise). Codec/engine upgrade constraints are documented in [docs/chromium-compatibility.md](docs/chromium-compatibility.md).

Audio capture follows [CEF's audio-handler API](https://cef-builds.spotifycdn.com/docs/116.0/classCefAudioHandler.html), resampled to 48 kHz and framed into 20 ms Opus packets. Receiver audio plays through a bounded Java Sound monitor. Periodic keyframes support late joins and recovery. Bounded media queues avoid accumulating unlimited delay. Actual FPS depends on Minecraft rendering, GPU/CPU capacity, and the connection.

Decoded video uploads straight to a render-thread RGBA texture from the decoder worker; only the newest frame is kept, so a slow viewer drops instead of lagging. The server sends a demand signal when the first compatible player enters the stream's dimension and when the last one leaves; sender-side capture and encoding stay closed at zero viewers while the lightweight placement heartbeat remains active. Capture cadence is enforced before encoding.
...
The audio handler retains the sample rate from `getAudioParameters`, because MCEF 2.1.6's native adapter passes null parameters to `onAudioStreamStarted`. Unknown parameters disable capture instead of guessing a sample rate. A stateful resampler preserves fractional phase across CEF callbacks and timestamps output from its cumulative 48 kHz frame count. The sender accumulates resampled PCM into 20 ms Opus frames; receiver-side Opus decodes into the same planar layout the local monitor already plays. Audio is preserved when video queues overflow, while stale video is discarded until a replacement keyframe.

Streaming writes rate-limited health summaries to `logs/latest.log` on both clients and the server. Every ten seconds these identify raw capture, selected H.264 acceleration, Opus encode/decode, relay packet and budget drops, keyframe waits, queue resets, and rendered frames. Linux capture also records the OpenGL vendor/renderer and sampled RGB range; a solid-color source frame is called out explicitly. Search the log for `Stream capture`, `Stream audio`, `Stream encoder`, `Stream decoder`, or `Stream relay` when reporting a failure.
...
The build runs automated media/control-envelope, permission, queue-bound, relay, geometry, and packaging checks. Native FFmpeg encode selection, GPU acceleration, cross-platform audio, remote input, and the Linux OpenGL capture path still require an in-game test on the target drivers.
## Controls
...
The taskbar's Apps button toggles a start menu directly above its left edge, in the same plane and orientation. It follows taskbar movement and recall; dragging either attached panel moves both together. Closing Tasks closes its attached menu.
...
Window picking respects player block-interaction reach and nearer world targets. The wheel moves distant panels in larger steps so they can be brought back quickly; nearby movement retains fine steps. Normal interaction uses Minecraft reach by default. Settings > Extend interaction range enables an adjacent distance field (default 16 blocks, positive values up to 4,096); the setting and distance persist. This applies to panel clicking, scrolling, and interaction targeting. Nearer blocks/entities still occlude targeting. Windows clear on death, world/dimension changes, and disconnect; browser views are closed. Inventory items persist. Window placement is not saved yet. Private apps remain local. Apps can be shared over LAN or a dedicated server as described above.

## Verification

For this update, test titlebar dragging/close on Browser and custom apps, title changes when switching tabs, taskbar grip dragging, Alt+middle and laser-middle drags from anywhere, instant Alt pill reveal, scroll up/down distance with the pill and the new drag methods, and fixed-size Tasks/Apps. Automated checks cover titlebar picking on rotated windows, close/drag boundaries, retained content resolution, resize corners, and distance direction/limits. Cursor/input and first-person model injection have a packaging gate for all 12 mappings; native gameplay still needs an in-game check.
...
1. Get controls: Tasks and Laser Pointer are given. Old Interact, Apps, and Window Drag items disappear from the inventory.
2. Open Apps through the taskbar button. Open Browser, then recall it without resetting its tabs.
...
Sender stream options include **Encoder**: Auto (GPU probe order, x264 fallback),
H.264 GPU (fail rather than silently using software), and H.264 CPU (x264 directly).
...
selection. Viewers have no encoder controls.
