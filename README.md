...
- Linux/macOS build: `./gradlew build` (or `sh gradlew build` if the executable bit was not preserved). The PowerShell helper below is for Windows.
...
- The script discovers Temurin and stores Gradle caches locally. An IDE can import build.gradle with JDK 21.
- Replace the old Prism mod JAR with `build/libs/winlandcraft-0.1.28-dev.jar`. Keep only one WinLandCraft version installed, alongside Fabric API and MCEF.
...
Settings apply live and persist in `config/winlandcraft.json`. The encoder uses VP9 video with periodic one-second keyframes and stereo 48 kHz Opus audio. The selected streamable-browser tab supplies audio through CEF's browser-scoped audio handler. Desktop audio, microphone input, private browsers, and inactive tabs are not broadcast. Because CEF capture replaces native audio output, a bounded Java Sound playback queue keeps the creator's selected tab audible even when broadcast audio is off.
...
Audio capture follows [CEF's audio-handler API](https://cef-builds.spotifycdn.com/docs/116.0/classCefAudioHandler.html). Playback timestamps share a clock, with a small receiver buffer and periodic keyframes for late joins and recovery. Bounded media queues discard stale data under load rather than accumulating an unlimited delay. Actual FPS depends on Minecraft rendering, Chromium, CPU/GPU capacity, and the connection.

Install this same build on **all clients and the world host/server**: the VP9 protocol is incompatible with the previous VP8 and JPEG builds. Clients require Fabric API and MCEF; dedicated servers require only Fabric API and WinLandCraft. Essential/LAN still use the integrated server's existing Minecraft connection. The server validates ownership and media envelopes, then forwards compressed bytes; it never runs Chromium or encodes/decodes media. This remains a TCP Minecraft relay, not WebRTC. Bitrate increases also increase the world's host/relay bandwidth usage for every viewer.

The audio handler retains the sample rate from `getAudioParameters`, because MCEF 2.1.6's native adapter passes null parameters to `onAudioStreamStarted`. This fixes the exception that previously prevented both creator playback and broadcast audio. Unknown parameters disable capture instead of guessing a sample rate.
...
This video/audio implementation was compiled and packaged only, as requested. No runtime, multiplayer, audio, or automated tests were executed for this revision; the earlier JPEG capture smoke test does not validate the new codec pipeline.
## Controls
