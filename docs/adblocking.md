...
The private `http://127.0.0.1:<ephemeral port>` WebCodecs bridge is excluded before any native filter query or cosmetic injection. During an initial JCEF navigation, subresources can temporarily report `about:blank` as their initiator. Brave's rules may correctly treat an otherwise harmless `worker.js` request from that unknown initiator as third-party advertising; applying that decision to WinLandCraft's own loopback script leaves both stream endpoints permanently at `phase=created`. Keep this exclusion limited to the literal IPv4 loopback host and plain HTTP used by `MediaBridge`; lookalike hostnames and normal web traffic must still be filtered.
