...
CEF callback blocks are resampled to 48 kHz with one continuous fractional phase; packet boundaries therefore cannot add or remove rounded samples. The sender timestamps them from the cumulative output-frame count. Encoded video and audio use bounded, numbered bridge batches with a small fixed concurrency window, and Java restores request order before handing packets to Minecraft. If a queue reaches its latency bound, it preserves audio, discards stale video, and waits for a new keyframe. The receiver schedules decoded audio contiguously on the `AudioContext` clock behind a 150 ms cushion. Ordinary timestamp jitter does not stop already scheduled sources; only an actual underrun re-establishes the cushion. Video presentation remains callback-driven because hidden OSR pages throttle JavaScript timers.
