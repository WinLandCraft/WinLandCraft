...
final class EdgeControls extends WorldPanel {
    ...
    void streamClick(int x,int y) {
        ...
        int local=y-streamY();
        if(local>=208&&local<236&&x>=12&&x<348){owner.streamClient.stop(owner);sync();return;}
        if(local>=168&&local<192&&owner.remoteControlAllowed()&&x>=292&&x<348) {
            ModSettings.streamRemoteControl=!ModSettings.streamRemoteControl;
            if(!ModSettings.streamRemoteControl)owner.clearRemoteControls();
        } else {
            if(local<24||local>=168||x<292||x>=348||(local-24)%24>=22)return;
            ...
            switch(row) {
                ...
                case 5->ModSettings.streamCodecMode=Math.clamp(ModSettings.streamCodecMode+step,0,StreamQuality.CODECS.length-1);
    ...
    private void drawStream(PanelCanvas c) {
        ...
        c.text(Minecraft.getInstance().font.plainSubstrByWidth(owner.streamStatus,326),18,top+196,0xFFD4ACFF);
