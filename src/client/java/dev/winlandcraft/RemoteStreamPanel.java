...
final class RemoteStreamPanel extends WorldPanel {
    ...
    void receive(byte[] packet) {
        ...
        decoder.receive(packet);lastFrame=System.currentTimeMillis();
    }
    @Override public void tick(Minecraft client) {
        super.tick(client);if(isOpen()&&decoder!=null)decoder.tick();
    }
    @Override public void render(net.fabricmc.fabric.api.client.rendering.v1.WorldRenderContext context) {
        try(var canvas=canvas(context)) {
            ...
            boolean image=decoder!=null&&decoder.browser!=null&&decoder.browser.getRenderer().getTextureID()>0;
            if(image)canvas.texture(texture,pixelWidth(),pixelHeight());
            else {canvas.rect(0,0,pixelWidth(),pixelHeight(),.1f,0xFF21182D);canvas.text("Connecting to shared browser...",20,40,-1,2);}
            ...
            else if(image&&System.currentTimeMillis()-lastFrame>5000) {
